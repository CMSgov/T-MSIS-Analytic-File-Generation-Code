/**********************************************************************************************/
/*Program: 007_bsf_ELG00007.sas
/*Author: Gerry Skurski, Mathematica Policy Research
/*Date: 3/2/2017
/*Purpose: Process TMSIS_HH_SNTRN_PRVDR and create unique output for BSF.
/*Mod: 
/*Notes: This program is included by 001_batch_bsf.sas
/**********************************************************************************************/
 
%macro create_ELG00007(tab_no, _2x_segment, eff_date, end_date);
%let created_vars=;
...
%MultiIds(sort_key=%str(coalesce(trim(HH_PRVDR_NUM),'xx')))
...
%Get_Audt_counts(&DA_SCHEMA.,&DA_RUN_ID., 007_bsf_ELG00007, 0.1. create_initial_table);
%Get_Audt_counts(&DA_SCHEMA.,&DA_RUN_ID., 007_bsf_ELG00007, 0.2. MultiIds);

%drop_table_multi(&tab_no. &tab_no._recCt &tab_no._uniq &tab_no._multi &tab_no._multi_all &tab_no._multi_step2);

%mend create_ELG00007;
